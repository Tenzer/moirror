...
server.get('/', function (req, res, next) {
  fs.readdir(__dirname + '/configs/', function (err, files) {
    if (err) {
      res.send(500, util.format('Error when getting files in config directory: %s', err));
      return next();
    }
...
server.get('/:config', function (req, res, next) {
  fs.readFile(__dirname + '/configs/' + req.params.config, function (err, data) {
    if (err) {
